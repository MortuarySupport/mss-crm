...
function CheckInFlow({user,cases,onComplete,onBack}) {
  ...
  const [form,setForm]=useState({firstName:"",lastName:"",dob:"",dod:"",sex:"",transferFrom:"",transferFromSub:"",transferredBy:"",transferDate:today(),paperwork:[],nilVals:false,vals:false,valsText:"",otherItems:[],otherText:"",size:"",fridge:"",weight:"",pacemaker:false});
  const isFD=user?.role==="fd",isTransfer=user?.role==="transfer";
  ...
  const effectiveStep=isFD?3:isTransfer&&step===1?2:step;
...
function VehicleLog({user,onBack}){
  ...
  function resetForm(){setVehicle("");setTripType("");setDriver(user?.name||"");setDepartDate(todaySydney());setDepartTime(new Date().toLocaleTimeString("en-AU",{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:"Australia/Sydney"}));setReturnDate(todaySydney());setReturnTime(new Date().toLocaleTimeString("en-AU",{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:"Australia/Sydney"}));setDepartFrom("");setDestinations([""]);setReturnLocation("");setMileStart("");setMileEnd("");setSigData(null);setHasSig(false);savedImageRef.current=null;}
  ...
  if(view==="new") return(
    <div className="max-w-lg mx-auto px-4 py-4">
      ...
      <div className="space-y-4">
        ...
        <div><div className="text-xs font-black uppercase text-gray-500 mb-2">Trip Type</div><div className="grid grid-cols-3 gap-2">{["ED","Transfer","Funeral","Miscellaneous","Personal"].map(t=><button key={t} type="button" onClick={()=>setTripType(t)} className={"py-2 rounded-xl border-2 text-xs font-black transition "+(tripType===t?"bg-gray-900 text-white border-gray-900":"border-gray-200 text-gray-600 hover:border-gray-700")}>{t}</button>)}</div></div>
                    ...
                    return(
                      <td key={date} style={{padding:"2px",verticalAlign:"top",border:"1px solid #e5e7eb",minWidth:"60px"}}>
                        {slots.map((s,si)=>{
                          ...
                          const bg=!isMine?"#f3f4f6":s.completed?"#dcfce7":colors[si%colors.length];
                          const brd=!isMine?"#e5e7eb":s.completed?"#16a34a":bords[si%bords.length];
                          return(
                            <div key={si} style={{marginBottom:"2px",padding:"2px 3px",backgroundColor:bg,border:`1.5px solid ${brd}`,borderRadius:"3px",opacity:isMine?1:0.5}}>
                              {!isMine?<div style={{fontSize:"8px",color:"#9ca3af",fontWeight:"700"}}>BUSY</div>:<>
                              <div style={{fontSize:"9px",fontWeight:"900",color:"#1f2937",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.label}</div>
                              <div style={{fontSize:"8px",color:"#6b7280",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.label2}</div>
                              <div style={{fontSize:"8px",color:"#9ca3af",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.staff}</div>
                              {s.completed&&<div style={{fontSize:"8px",color:"#16a34a",fontWeight:"900"}}>✓ DONE</div>}
                              {!readOnly&&<div style={{display:"flex",gap:"1px",marginTop:"1px"}}>
                                <button onClick={()=>openEdit(s.booking)} style={{flex:1,fontSize:"7px",padding:"1px",background:"#e5e7eb",border:"none",borderRadius:"2px",cursor:"pointer",fontWeight:"700"}}>EDIT</button>
                                {!s.completed&&<button onClick={()=>setShowCompleteModal(s.booking)} style={{flex:1,fontSize:"7px",padding:"1px",background:"#111",color:"#fff",border:"none",borderRadius:"2px",cursor:"pointer",fontWeight:"700"}}>DONE</button>}
                              </div>}
                              </>}
                            </div>
                          );
