  ...
  return(
    ...
    <div>
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-1 mb-3">
        ...
        <button onClick={()=>updStatus("photo",si.photo===undefined?null:undefined)} className={"py-2 rounded-xl border-2 text-xs font-black uppercase transition "+(si.photo==="received"?green:neutral)}>{si.photo==="received"?"PHOTO ✓":"PHOTO"}</button>
      </div>
...
function VehicleWeekView({weekDates,vehicleBookings,onAddVehicleBooking,onUpdateVehicleBooking,onDeleteVehicleBooking,user,cases}){
  ...
  async function saveBooking(){
    ...
    try{
      const selCase=activeCases.find(x=>x.id===caseId);
      const data={jobType,date:jobDate,time:jobTime,hours:jobHours,staff:assignedStaff,case_id:tbc?"":caseId,tbc,case_label:tbc?"TBC":selCase?`${(selCase.lastName||"").toUpperCase()}, ${selCase.firstName}`:"",funeral_home:selCase?.funeralHomeName||"",notes,completed:false,created_by:user?.name||"",created_at:new Date().toISOString()};
      if(editingBooking){
